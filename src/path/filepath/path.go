...
func Walk(root string, walkFn WalkFunc) error { // 遍历目录，执行walkFn, root为根目录
	info, err := os.Lstat(root)
	if err != nil {
		return walkFn(root, nil, err) // 如果获得文件信息
	}
...
func Base(path string) string { // 返回路径名的最后一项
	if path == "" { // 如果path为空，返回当前路径
		return "."
	}
	// Strip trailing slashes.
	for len(path) > 0 && os.IsPathSeparator(path[len(path)-1]) { // 如果有的话去掉最后一个文件分隔符
		path = path[0 : len(path)-1]
	}
	// Throw away volume name
	path = path[len(VolumeName(path)):] // 去掉卷名
...
func Dir(path string) string { // 返回路径的目录名
	vol := VolumeName(path)
	i := len(path) - 1
	for i >= len(vol) && !os.IsPathSeparator(path[i]) {
		i--
