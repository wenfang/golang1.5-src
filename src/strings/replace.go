...
type Replacer struct { // 对replacer接口的包装
	r replacer
}

// replacer is the interface that a replacement algorithm needs to implement.
type replacer interface { // replacer接口，需要实现Replace和WriteString两个方法
...
func NewReplacer(oldnew ...string) *Replacer { // 新建一个Replacer
	if len(oldnew)%2 == 1 { // 必须接受偶数个参数
...
func (r *Replacer) Replace(s string) string { // 实现字符串替换
	return r.r.Replace(s)
}

// WriteString writes s to w with all replacements performed.
func (r *Replacer) WriteString(w io.Writer, s string) (n int, err error) { // 实现字符串替换后输出
