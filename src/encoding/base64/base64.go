...
func NewEncoding(encoder string) *Encoding { // 新创建一个base64编码器
	...
	for i := 0; i < len(encoder); i++ { // 遍历每个字符
...
// RFC 4648.
var StdEncoding = NewEncoding(encodeStd) // 标准的base64 encode

// URLEncoding is the alternate base64 encoding defined in RFC 4648.
// It is typically used in URLs and file names.
var URLEncoding = NewEncoding(encodeURL) // 用作URL的base64编码器
...
func (enc *Encoding) Encode(dst, src []byte) { // 将src编码为dst输出
	if len(src) == 0 { // 如果src长度为0，直接返回
		return
	}
...
func (enc *Encoding) EncodeToString(src []byte) string { // 将src编码为string输出
...
func NewEncoder(enc *Encoding, w io.Writer) io.WriteCloser { // 新创建一个Encoder
...
func NewDecoder(enc *Encoding, r io.Reader) io.Reader { // 新创建一个Decoder
