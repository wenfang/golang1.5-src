...
type Encoder struct {
	mutex      sync.Mutex              // each item must be sent atomically 每个元素必须序列化发送
	w          []io.Writer             // where to send the data
	sent       map[reflect.Type]typeId // which types we've already sent 已经发送了什么类型
...
func NewEncoder(w io.Writer) *Encoder { // 新创建一个Encoder编码器
	enc := new(Encoder)    // 创建Encoder结构
	enc.w = []io.Writer{w} // Writer列表，是Slice
...
func (enc *Encoder) pushWriter(w io.Writer) { // 添加一个writer到encoder
...
func (enc *Encoder) Encode(e interface{}) error { // 将e进行编码，调用EncodeValue
...
func (enc *Encoder) EncodeValue(value reflect.Value) error {
	...
	if value.Kind() == reflect.Ptr && value.IsNil() { // 是指针类型或者nil，无法进行encode
	...
	enc.mutex.Lock()
	defer enc.mutex.Unlock() // 先加锁

	// Remove any nested writers remaining due to previous errors.
	enc.w = enc.w[0:1] // 先只看第一个Writer
