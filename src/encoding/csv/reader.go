...
func NewReader(r io.Reader) *Reader { // 新创建一个csv文件的Reader
	return &Reader{
		Comma: ',', // reader的分隔符
		r:     bufio.NewReader(r),
	}
...
func (r *Reader) error(err error) error { // 基于err创建一个ParseError
	return &ParseError{ // 返回一个解析错误结构
		Line:   r.line,
		Column: r.column,
		Err:    err,
	}
...
func (r *Reader) Read() (record []string, err error) { // 读一串记录
	for {
