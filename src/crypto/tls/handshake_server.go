...
type serverHandshakeState struct { // 服务端握手状态
	c               *Conn
...
func (c *Conn) serverHandshake() error { // 作为服务端处理握手
	config := c.config

	// If this is the first server handshake, we generate a random key to
	// encrypt the tickets with.
	config.serverInitOnce.Do(config.serverInit) // server端初始化一次

	hs := serverHandshakeState{ // 初始化一个服务端握手状态结构
		c: c,
	}
	isResume, err := hs.readClientHello() // 读客户端发送的Hello
	if err != nil {
		return err
	}

	// For an overview of TLS handshaking, see https://tools.ietf.org/html/rfc5246#section-7.3
	if isResume { // client端包含session ticket做一个简略的握手
		...
		c.didResume = true
	} else { // client端不包含session ticket做一个完全的握手
	...
	c.handshakeComplete = true // 设置握手完成
...
func (hs *serverHandshakeState) readClientHello() (isResume bool, err error) { // 读客户端发来的Hello消息
	config := hs.c.config
	c := hs.c
	...
	var ok bool
	hs.clientHello, ok = msg.(*clientHelloMsg) // 如果不是clientHelloMsg返回错误
	if !ok {
...
Curves:
	...
	_, err = io.ReadFull(config.rand(), hs.hello.random) // 生成server端的随机数
	if err != nil {
