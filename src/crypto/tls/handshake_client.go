...
func (c *Conn) clientHandshake() error { // 执行客户端握手
	...
	if len(c.config.ServerName) == 0 && !c.config.InsecureSkipVerify { // ServerName和InsecureSkipVerify必须设定
...
NextCipherSuite:
	...
	_, err := io.ReadFull(c.config.rand(), hello.random) // 从随机数发生器中获取随机数
	if err != nil {
