...
func Request() (*http.Request, error) { //
	r, err := RequestFromMap(envMap(os.Environ())) // 先将环境变量字符串变为key/value对
	if err != nil {
		return nil, err
	}
...
func envMap(env []string) map[string]string { // 将环境变量字符串变为key/value对
...
func RequestFromMap(params map[string]string) (*http.Request, error) { // 根据环境参数读取为http请求
	r := new(http.Request)
