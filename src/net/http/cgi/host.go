...
var trailingPort = regexp.MustCompile(`:([0-9]+)$`) // 在CGI服务器中使用的端口号

var osDefaultInheritEnv = map[string][]string{ // 对不同平台共享库默认的环境变量
...
type Handler struct { // 在CGI环境的子进程中执行
	Path string // path to the CGI executable 执行文件的路径
	Root string // root URI prefix of handler or empty for "/" 根目录
...
func (h *Handler) ServeHTTP(rw http.ResponseWriter, req *http.Request) { // 将HTTP请求转换为CGI请求，并执行脚本
	root := h.Root
	if root == "" {
		root = "/"
	}
