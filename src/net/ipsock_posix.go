...
func probeIPv6Stack() (supportsIPv6, supportsIPv4map bool) { // 探测ipv6协议栈，是否支持ipv6 mapping
	var probes = []struct { // 探测两个地址
		laddr TCPAddr
		value int
	}{
	...
	for i := range probes { // 遍历探测地址
		...
		if err := syscall.Bind(s, sa); err != nil { // 绑定地址
			continue
		}
		supps[i] = true
	}
...
func favoriteAddrFamily(net string, laddr, raddr sockaddr, mode string) (family int, ipv6only bool) { // 返回最合适的地址类型
	switch net[len(net)-1] { // 查看net最后一位，如果是4或者6，返回响应期望的地址类型
	case '4':
	...
	if mode == "listen" && (laddr == nil || laddr.isWildcard()) { // 如果是listen，但是没有本地地址或者是统配地址
		if supportsIPv4map { // 如果支持ipv4，ipv6地址的映射
			return syscall.AF_INET6, false // 返回ipv6地址
		}
		if laddr == nil { // 如果本地地址为空，返回AF_INET
			return syscall.AF_INET, false
		}
...
func internetSocket(net string, laddr, raddr sockaddr, deadline time.Time, sotype, proto int, mode string) (fd *netFD, err error) {
	...
	return socket(net, family, sotype, proto, ipv6only, laddr, raddr, deadline) // 创建一个socket
}
