...
func newUDPConn(fd *netFD) *UDPConn { return &UDPConn{conn{fd}} } // 创建一个新的UDP连接
...
func (c *UDPConn) ReadFrom(b []byte) (int, Addr, error) {
	...
	n, addr, err := c.ReadFromUDP(b) // 调用ReadFromUDP
	if addr == nil {
		return n, nil, err
	}
	return n, addr, err
...
// addressing.
func ListenUDP(net string, laddr *UDPAddr) (*UDPConn, error) { // 监听udp端口，创建一个udp socket
	switch net { // 网络类型必须为udp
	case "udp", "udp4", "udp6":
	default: // 如果网络类型不是udp返回错误
		return nil, &OpError{Op: "listen", Net: net, Source: nil, Addr: laddr.opAddr(), Err: UnknownNetworkError(net)}
	}
	if laddr == nil { // 如果本地地址没有设置，设置为空
		laddr = &UDPAddr{}
	}
	...
	return newUDPConn(fd), nil // 返回新的UDP连接
}
...
func ListenMulticastUDP(network string, ifi *Interface, gaddr *UDPAddr) (*UDPConn, error) {
	...
	c := newUDPConn(fd)                    // 创建一个udp连接
	if ip4 := gaddr.IP.To4(); ip4 != nil { // 如果是ipv4地址
