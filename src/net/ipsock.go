...
func SplitHostPort(hostport string) (host, port string, err error) { // 分割host和port
	j, k := 0, 0
...
// "[host]:port".
func JoinHostPort(host, port string) string { // 生成host:port的形式，如果host中包含:或%，生成[host]:port的形式
...
func internetAddrList(net, addr string, deadline time.Time) (addrList, error) {
	...
	switch net { // 根据网络的类型，分割出主机和端口号
	case "tcp", "tcp4", "tcp6", "udp", "udp4", "udp6":
		if addr != "" {
			if host, port, err = SplitHostPort(addr); err != nil { // 分裂成主机和端口号
				return nil, err
			}
			if portnum, err = parsePort(net, port); err != nil { // 获得端口号
				return nil, err
			}
		}
	case "ip", "ip4", "ip6":
