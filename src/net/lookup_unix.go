...
// for quick access.
func readProtocols() {
	if file, err := open("/etc/protocols"); err == nil { // 读protocols文件
		for line, ok := file.readLine(); ok; line, ok = file.readLine() { // 读文件的每一行
			// tcp    6   TCP    # transmission control protocol
			if i := byteIndex(line, '#'); i >= 0 { // 忽略注释
				line = line[0:i]
			}
			f := getFields(line) // 分割域
			if len(f) < 2 {
				continue
			}
...
func lookupProtocol(name string) (int, error) {
	onceReadProtocols.Do(readProtocols) // 读一次协议文件
	proto, found := protocols[name]     // 根据协议名返回协议号
	if !found {
