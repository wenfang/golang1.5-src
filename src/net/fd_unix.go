...
func dial(network string, ra Addr, dialer func(time.Time) (Conn, error), deadline time.Time) (Conn, error) { // 执行dial连接
	return dialer(deadline)
}

func newFD(sysfd, family, sotype int, net string) (*netFD, error) { // 新创建一个netFD结构
	return &netFD{sysfd: sysfd, family: family, sotype: sotype, net: net}, nil
}

func (fd *netFD) init() error { // 初始化，执行pollDesc的Init
...
func (fd *netFD) setAddr(laddr, raddr Addr) { // 设置netFD的本地地址和远程地址
	fd.laddr = laddr
	fd.raddr = raddr
	runtime.SetFinalizer(fd, (*netFD).Close) // 设置finalizer运行Close
}
...
func (fd *netFD) incref() error { // 增加对fd的引用计数
	if !fd.fdmu.Incref() {
		return errClosing
	}
	return nil
...
func (fd *netFD) decref() { // 减少对fd的引用计数
	if fd.fdmu.Decref() {
		fd.destroy()
	}
}
...
func (fd *netFD) shutdown(how int) error {
	...
	defer fd.decref() // defer的时候减少引用计数
	return os.NewSyscallError("shutdown", syscall.Shutdown(fd.sysfd, how))
}

func (fd *netFD) closeRead() error { // 关闭读端
	return fd.shutdown(syscall.SHUT_RD)
}

func (fd *netFD) closeWrite() error { // 关闭写端
...
func (fd *netFD) Write(p []byte) (nn int, err error) { // 写时加写锁
	if err := fd.writeLock(); err != nil { // 对每个fd的写进行序列化
		return 0, err
	}
