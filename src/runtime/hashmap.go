...
const (
	...
	dataOffset = unsafe.Offsetof(struct { // 数据偏移量，需要为bmap结构的大小
		b bmap
		v int64
	}{}.v)
	// tophash可能的取值
	...
	empty          = 0 // cell is empty 当前cell为空
	evacuatedEmpty = 1 // cell is empty, bucket is evacuated. 当前cell为空,bucket正在进行evacuated
...
type bmap struct { // 一个Go map的bucket
	tophash  [bucketCnt]uint8 // 8个字节的tophash
...
func evacuated(b *bmap) bool { // 判断当前bucket是否正在evacuated
	h := b.tophash[0]
...
func makemap(t *maptype, hint int64, h *hmap, bucket unsafe.Pointer) *hmap {
	...
	if hint < 0 || int64(int32(hint)) != hint { // 指定map中元素的个数有误，panic
		panic("makemap: size out of range")
		// TODO: make hint an int, then none of this nonsense
	}

	if !ismapkey(t.key) { // map的key不能作为key类型，抛出异常
	...
	if t.key.align > bucketCnt { // key对齐大小错误
		throw("key align too big")
	}
	if t.elem.align > bucketCnt { // value对齐大小错误
		throw("value align too big")
	}
	if uintptr(t.key.size)%uintptr(t.key.align) != 0 { // key不是对齐大小的整数倍
		throw("key size not a multiple of key align")
	}
	if uintptr(t.elem.size)%uintptr(t.elem.align) != 0 { // value不是对齐大小的整数倍
		throw("value size not a multiple of value align")
	}
	if bucketCnt < 8 { // bucket中元素的数量太少了
...
func mapaccess1(t *maptype, h *hmap, key unsafe.Pointer) unsafe.Pointer { // 返回指向h[key]的指针，从来不会返回NUL，如果key没有在map中返回一个队zero对象的引用
	...
	if h == nil || h.count == 0 { // 如果hmap为空，返回指向zero对象的指针
	...
	if c := h.oldbuckets; c != nil {
		...
		if !evacuated(oldb) { // 如果oldb bmap没在进行evacuated
			b = oldb
		}
	}
...
func mapassign1(t *maptype, h *hmap, key unsafe.Pointer, val unsafe.Pointer) { // 将key和value赋值给一个map
	if h == nil { // 如果map为空，panic
...
again:
	bucket := hash & (uintptr(1)<<h.B - 1) // 获得bucket号
	if h.oldbuckets != nil {
		growWork(t, h, bucket)
	}
	b := (*bmap)(unsafe.Pointer(uintptr(h.buckets) + bucket*uintptr(t.bucketsize))) // 定位到bmap
	top := uint8(hash >> (ptrSize*8 - 8))
	if top < minTopHash {
		top += minTopHash
	}
	...
	for {
		for i := uintptr(0); i < bucketCnt; i++ { // 遍历所有的bucketmap
			if b.tophash[i] != top {
...
func mapiterinit(t *maptype, h *hmap, it *hiter) { // 初始化map迭代器
...
func mapiternext(it *hiter) { // 在map中迭代下一项
	h := it.h
	if raceenabled {
