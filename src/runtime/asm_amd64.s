...
TEXT runtime·rt0_go(SB),NOSPLIT,$0 // 程序的进入点
	// copy arguments forward on an even stack 拷贝参数到栈上
	MOVQ	DI, AX		// argc 获得参数个数，放入AX
	MOVQ	SI, BX		// argv 获得参数列表的指针，放入BX
	SUBQ	$(4*8+7), SP		// 2args 2auto 在栈上为参数腾出空间，减少SP的值
	ANDQ	$~15, SP    // 将SP对齐，使栈中可以保存4*8个字节的数据
	MOVQ	AX, 16(SP)
	MOVQ	BX, 24(SP) // 将argc和argv拷贝到栈上
...
notintel:
	...
	MOVL	DX, runtime·cpuid_edx(SB) // 为CPU信息赋值
nocpuinfo:	
	
	// if there is an _cgo_init, call it.
	MOVQ	_cgo_init(SB), AX // 如果有_cgo_init，调用_cgo_init
	TESTQ	AX, AX // 检查_cgo_init是否存在
	JZ	needtls   // 没有跳转到needtls
	// g0 already in DI g0已经在DI中了，赋给CX
	MOVQ	DI, CX	// Win64 uses CX for first parameter
	MOVQ	$setg_gcc<>(SB), SI
	CALL	AX  // 调用_cgo_init，cgo_init会更新stack_lo

	// update stackguard after _cgo_init
	MOVQ	$runtime·g0(SB), CX
	MOVQ	(g_stack+stack_lo)(CX), AX // 先取出来stack_lo
	ADDQ	$const_StackGuard, AX
	MOVQ	AX, g_stackguard0(CX)
	MOVQ	AX, g_stackguard1(CX) // 更新stackguard

	CMPL	runtime·iswindows(SB), $0 // 如果是windows上，忽略TLS，直接跳转到ok
	JEQ ok
needtls:
	// skip TLS setup on Plan 9
	CMPL	runtime·isplan9(SB), $1 // 在plan9上忽略TLS设置，如果是Plan9系统，直接跳转到ok
	JEQ ok
	// skip TLS setup on Solaris
	CMPL	runtime·issolaris(SB), $1 // 如果是solaris系统，也直接跳转到ok
	JEQ ok
	...
	MOVQ	$0x123, g(BX) // 先将0x123保存一下
	MOVQ	runtime·tls0(SB), AX  // 再将值取回到AX中
	CMPQ	AX, $0x123
	JEQ 2(PC) // 如果成功，跳跃两条指令，也就是开始执行ok
	MOVL	AX, 0	// abort
ok:
	...
	MOVQ	CX, m_g0(AX)
	// save m0 to g0->m 设置g0的m为m0
	MOVQ	AX, g_m(CX)

	CLD				// convention is D is always left cleared
	CALL	runtime·check(SB)

	MOVL	16(SP), AX		// copy argc 拷贝参数
	MOVL	AX, 0(SP)
	MOVQ	24(SP), AX		// copy argv
	MOVQ	AX, 8(SP)
	CALL	runtime·args(SB)
	CALL	runtime·osinit(SB) // 获得cpu的数量
	CALL	runtime·schedinit(SB) // 初始化调度器，初始化P

	// create a new goroutine to start program
	MOVQ	$runtime·mainPC(SB), AX		// entry
	PUSHQ	AX
	PUSHQ	$0			// arg size
	CALL	runtime·newproc(SB) // 创建主goroutine，执行main函数
	POPQ	AX
	POPQ	AX

	// start this M
	CALL	runtime·mstart(SB) // 调用mstart，一去不回头
...
// void gosave(Gobuf*)
// save state in Gobuf; setjmp  将goroutine的状态保存在Gobuf中，保存sp，pc与当前的g
TEXT runtime·gosave(SB), NOSPLIT, $0-8
	MOVQ	buf+0(FP), AX		// gobuf
	LEAQ	buf+0(FP), BX		// caller's SP
	MOVQ	BX, gobuf_sp(AX) // 将SP保存到Gobuf的sp域中
	MOVQ	0(SP), BX		// caller's PC // 获取调用者的PC
	MOVQ	BX, gobuf_pc(AX) // 保存到Gobuf的PC
	MOVQ	$0, gobuf_ret(AX)
	MOVQ	$0, gobuf_ctxt(AX)
	MOVQ	BP, gobuf_bp(AX)
	get_tls(CX) // 获取tls中的g
	MOVQ	g(CX), BX
	MOVQ	BX, gobuf_g(AX) // 把g保存到Gobuf
	RET

// void gogo(Gobuf*)
// restore state from Gobuf; longjmp 从Gobuf中恢复goroutine的运行状态
...
TEXT runtime·mcall(SB), NOSPLIT, $0-8
	...
	MOVQ	AX, (g_sched+gobuf_g)(AX) // 保存调用者的状态到g->gobuf中
	...
	MOVQ	g_m(BX), BX
	MOVQ	m_g0(BX), SI
	CMPQ	SI, AX	// if g == m->g0 call badmcall 如果当前已经在g0的栈中了，调用badmcall
	JNE	3(PC)
...
TEXT runtime·morestack(SB),NOSPLIT,$0-0
	// Cannot grow scheduler stack (m->g0). 不能grow调度器的栈，也就是g0的栈
	get_tls(CX)
	MOVQ	g(CX), BX
	MOVQ	g_m(BX), BX
	MOVQ	m_g0(BX), SI // 获取g0
	CMPQ	g(CX), SI
	JNE	2(PC)
	INT	$3 // 如果在g0的栈上，调用3号中断
	...
	MOVQ	8(SP), AX	// f's caller's PC // 把调用者的PC值赋值给AX
	MOVQ	AX, (m_morebuf+gobuf_pc)(BX) // 保存PC值到gobuf_pc
	LEAQ	16(SP), AX	// f's caller's SP
	MOVQ	AX, (m_morebuf+gobuf_sp)(BX) // 保存SP值到gobuf_sp
	get_tls(CX)
	MOVQ	g(CX), SI
...
TEXT runtime·aeshash(SB),NOSPLIT,$0-32 // 使用AES硬件指令的hash函数
...
TEXT runtime·goexit(SB),NOSPLIT,$0-0 // 调用goexit1，不会返回
	BYTE	$0x90	// NOP
