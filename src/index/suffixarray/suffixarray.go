...
type Index struct { // 索引结构实现快速子字符串查找
	data []byte
	sa   []int // suffix array for data; len(sa) == len(data) data的前缀数组
}

// New creates a new Index for data.
// Index creation time is O(N*log(N)) for N = len(data).
func New(data []byte) *Index { // 创建索引结构，索引数据
...
func (x *Index) Read(r io.Reader) error { // 从r中读索引x的数据
...
func (x *Index) Write(w io.Writer) error { // 将索引x的数据写入w
