...
var flateWriterPool sync.Pool // 创建一个flateWriter的Pool
...
func RegisterDecompressor(method uint16, d Decompressor) { // 注册解压函数
	mu.Lock()
	defer mu.Unlock()
...
func RegisterCompressor(method uint16, comp Compressor) { // 注册压缩函数
	mu.Lock()
	defer mu.Unlock()
...
func compressor(method uint16) Compressor { // 返回压缩函数
	mu.RLock()
	defer mu.RUnlock()
	return compressors[method]
}

func decompressor(method uint16) Decompressor { // 返回解压函数
	mu.RLock()
	defer mu.RUnlock()
