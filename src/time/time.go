...
type Time struct { // 对应epoch的时间结构
	...
	loc *Location // 指定对应Time的location
}

// After reports whether the time instant t is after u.
func (t Time) After(u Time) bool { // 检查时间t是否在时间u之后
	return t.sec > u.sec || t.sec == u.sec && t.nsec > u.nsec
}

// Before reports whether the time instant t is before u.
func (t Time) Before(u Time) bool { // 检查时间t是否在时间t之前
	return t.sec < u.sec || t.sec == u.sec && t.nsec < u.nsec
}
...
// the locations.
func (t Time) Equal(u Time) bool { // 检查时间t是否与时间u相等
...
func (m Month) String() string { return months[m-1] } // 将月份转换为字符串
...
func (d Weekday) String() string { return days[d] } // 将星期转换为字符串
...
// January 1, year 1, 00:00:00 UTC.
func (t Time) IsZero() bool { // 判断时间是否为空
	return t.sec == 0 && t.nsec == 0
}

// abs returns the time t as an absolute time, adjusted by the zone offset.
// It is called when computing a presentation property like Month or Hour.
func (t Time) abs() uint64 { // 获取绝对时间，秒数
	l := t.loc
...
func (t Time) Date() (year int, month Month, day int) { // 返回时间所表示的年月日
	year, month, day, _ = t.date(true)
	return
}

// Year returns the year in which t occurs.
func (t Time) Year() int { // 返回时间对应的年份
	year, _, _, _ := t.date(false)
	return year
}

// Month returns the month of the year specified by t.
func (t Time) Month() Month { // 返回对应的月份
	_, month, _, _ := t.date(true)
	return month
}

// Day returns the day of the month specified by t.
func (t Time) Day() int { // 返回对应的天
	_, _, day, _ := t.date(true)
	return day
}

// Weekday returns the day of the week specified by t.
func (t Time) Weekday() Weekday { // 返回对应的星期
...
// of year n+1.
func (t Time) ISOWeek() (year, week int) { // 返回年对应的周
...
func (t Time) Clock() (hour, min, sec int) { // 返回时间所表示的时分秒
...
// with no unit.
func (d Duration) String() string { // 将Duration转换为字符串的形式
...
func now() (sec int64, nsec int32) // 返回当前时间
...
func (t Time) GobEncode() ([]byte, error) { // encode成gob
	return t.MarshalBinary()
}

// GobDecode implements the gob.GobDecoder interface.
func (t *Time) GobDecode(data []byte) error { // 从gobdecode出来
...
func Unix(sec int64, nsec int64) Time { // 获得对应sec和nsec的时间结构
	if nsec < 0 || nsec >= 1e9 {
		n := nsec / 1e9
		sec += n
		nsec -= n * 1e9
